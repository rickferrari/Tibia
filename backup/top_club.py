...
##print(worlds)
     #Coletando categoria
cursor.execute("""select nome from category where nome='club'""")
...
for w in worlds:
        for p in range(1,2): # Para club, pegar apenas a primeira página
...
##            print(cols)
            if len(cols[0]) < 3: #Eliminando a linha de "titulo"
                    club.append([ele for ele in cols if ele]) #Livrar-se de valores vazios
        lista = list(club)
##print(club[1])


        #Inset data in table
cursor.execute("""DELETE FROM top_club""")
conn.commit()
cursor.execute("""UPDATE SQLITE_SEQUENCE SET SEQ=0 WHERE NAME='top_club';""")
conn.commit()
for dmain in club:
##        print(dmain)
        cursor.execute("""
        INSERT INTO top_club (Rank, Name, Vocation, Level, World, Link, Extract_data)
        VALUES (?,?,?,?,?,?,?)""",(dmain))
conn.commit()
